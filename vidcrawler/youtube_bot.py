...
# Example HTML for an individual video.
"""
...
def parse_youtube_videos(content: str) -> list[YtVid]:
    """Parse the HTML of a youtube channel and return a list of videos."""
    pattern_url = re.compile(r'href="/watch\?v=(.*?)"')
    urls = pattern_url.findall(content)
    # uniquify
    urls = list(set(urls))
    # generate full youtube urls from the ids
    urls = [f"https://www.youtube.com/watch?v={url}" for url in urls]
    out = [YtVid(url) for url in urls]
    return out
